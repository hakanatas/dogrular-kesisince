...
async function exportFormat(browser, format, ffmpeg) {
  ...
  const base = `dogrular-kesisince_${format}_${CAPS}`;
  ...
  const enc = spawn(ffmpeg, ['-y', '-loglevel', 'error', '-f', 'image2pipe', '-framerate', String(FPS), '-c:v', FRAMES === 'jpeg' ? 'mjpeg' : 'png', '-i', '-',
    ...
    '-metadata', 'title=Doğrular Kesişince — Kesişen Doğrular ve Açılar', mp4], { stdio: ['pipe', 'inherit', 'inherit'] });
...
console.log(`Doğrular Kesişince → MP4  (${FPS} fps, captions: ${CAPS}, ${WORKERS} workers)`);
